...
elseif ($action=='act_edit_sidebar')
{
	...
	{
		sys_message('请填写边栏标题',$referer_url,2);
	}

	if (empty($module_content))
	{
		sys_message('请填写边栏内容',$referer_url,2);
	}
	...
	{
		sys_message('编辑插件失败，请重新返回添加',$referer_url,2);
	}
}
...
elseif ($action=='sidebar_setup_list')
{
	...
	/* 获得可安装的边栏模块 */
	$modules = array();
	$sidebar_dir        = @opendir(LSHX_BLOG_ROOT . '/includes/modules/sidebar/');
	while ($file = readdir($sidebar_dir))
	{
		if ($file != '.' && $file != '..' && file_exists(LSHX_BLOG_ROOT. '/includes/modules/sidebar/' . $file))
		{
			include_once(LSHX_BLOG_ROOT. '/includes/modules/sidebar/' . $file);
