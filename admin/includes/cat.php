...
elseif ($action=='del_cat')
{
	...
	if ($db->getone($sql)>0) {
		sys_message('该分类下还有日志，请先删除该分类下的日志',$referer_url,2);
	}
...
elseif ($action=='act_add_cat')
{
	...
	if (empty($cat_name))
	{
		sys_message('分类名字不能为空',$referer_url,2);
	}
	...
	{
		sys_message('添加分类失败，请重新返回添加','admin.php?act=add_cat',2);
	}
}
...
elseif ($action=='edit_cat')
{
	...
	if (empty($cat_id))
	{
		sys_message('分类id不能为空',$referer_url,2);
	}
	...
	{
		sys_message('读取分类数据失败，请返回重新修改',$referer_url,2);
	}
...
elseif ($action=='act_edit_cat')
{
	...
	{
		sys_message('修改分类失败，请重新返回添加','admin.php?act=edit_cat&id='.$cat_id,2);
	}
}
