...
if ($action=='blog_list')
{
	...
	if (is_numeric($keywords))
	{
		...
		if ($row=$db->getrow($sql))
		{
			...
			{
				sys_message('您没有权限编辑他人日志',$referer_url,2);
			}
			else
			{
...
elseif ($action=='del_blog')
{
	...
	if ($blog_id>0)
	{
		...
		{
			sys_message('删除博客失败，请重新删除',$referer_url,2);
		}
	}
...
elseif ($action=='add_blog')
{
	...
	$smarty->assign('u',str_replace(LSHX_BLOG_WS_ADMIN, '', dirname($url)));
...
elseif ($action=='act_add_blog')
{
	...
	if (empty($blog_title))
	{
		sys_message('博客标题不能为空',$referer_url,2);
	}
	$cat_id=intval($_POST['cat']);
	if (empty($blog_title))
	{
		sys_message('请选择分类',$referer_url,2);
	}
	...
	if ($url_type==2)
	{
		...
		if (!empty($url_type))
		{
			...
			{
				sys_message('您定义的URL已经存在于其他日志中，请返回重新定义',$referer_url,2);
			}
		}
	}
	...
	{
		sys_message('添加日志失败，请重新返回添加','admin.php?act=add_blog',2);
	}
}
...
elseif ($action=='edit_blog')
{
	...
	{
		sys_message('读取日志数据失败，请返回重新修改',$referer_url,2);
	}
...
elseif ($action=='act_edit_blog')
{
	...
	if (empty($blog_id))
	{
		sys_message('日志id不能为空',$referer_url,2);
	}
	...
	{
		sys_message('错误日志，请返回重新修改',$referer_url,2);
	}
	...
	{
		sys_message('修改日志失败，请重新返回添加','admin.php?act=edit_blog&id='.$blog_id,2);
	}
}
