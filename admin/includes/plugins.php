...
elseif ($action=='plugins_setup_list')
{
	...
	/* 获得可安装的插件 */
	$modules = array();
	$plugins_dir        = @opendir(LSHX_BLOG_ROOT . '/home/plugins/');
	while ($file = readdir($plugins_dir))
	{
		if ($file != '.' && $file != '..' && file_exists(LSHX_BLOG_ROOT. '/home/plugins/' . $file))
		{
			include_once(LSHX_BLOG_ROOT. '/home/plugins/' . $file.'/info.php');
...
elseif ($action=='setup_plugin')
{
	...
	/* 获得可安装的插件 */
	include_once(LSHX_BLOG_ROOT. '/home/plugins/' . $plugin_id.'/info.php');
	//如果插件已经安装或者不是插件
	...
	" (`plugin_id` ,`plugin_name` ,`plugin_desc` ,`author` ,`version` ,`plugin_point` ,`cp_type`,`add_time`)
		...
		{
			if (include_once(LSHX_BLOG_ROOT. '/home/plugins/' . $plugin_id.'/do.php'))
			{
		...
		VALUES (NULL , '".$modules['id']."', '".$modules['title']."', '".$desc."', '".$content."', '10', 1,'".$plugin_id."')";
					...
					{
						sys_message('安装插件边栏部分失败，请重新返回添加',$referer_url,2);
					}
				}
...
elseif ($action=='del_plugin')
{
	...
	if (!($master_db?$db2->query($sql):$db->query($sql)))
	{
		sys_message('删除失败，请重新删除',$referer_url,2);
	}
	else
	{
		/* 获得插件信息 */
...
{
	$plugin_id=$_GET['id'];
	include_once(LSHX_BLOG_ROOT. '/home/plugins/' . $plugin_id.'/admin_cp.php');
