...
elseif ($action=='del_page')
{
	...
	{
		sys_message('删除页面失败，请重新删除',$referer_url,2);
	}
}
...
elseif ($action=='act_add_page')
{
	...
	if (empty($page_title))
	{
		sys_message('页面标题不能为空',$referer_url,2);
	}
	...
	"VALUES (NULL , NULL, '$user_id', '".$page_title."', '".$desc."', '".$content."','$time', '$ip', '0','0' ) ";
	...
	{
		sys_message('创建页面失败，请重新返回添加','admin.php?act=add_page',2);
	}
}
...
elseif ($action=='edit_page')
{
	...
	{
		sys_message('读取页面数据失败，请返回重新修改',$referer_url,2);
	}
...
elseif ($action=='act_edit_page')
{
	...
	if (empty($page_id))
	{
		sys_message('页面id不能为空',$referer_url,2);
	}
	...
	{
		sys_message('修改页面失败，请重新返回添加','admin.php?act=edit_page&id='.$page_id,2);
	}
}
