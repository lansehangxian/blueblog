...
if ($action=='index') $action='default';
...
if ($action=='default')
{
	...
	//版权信息
	$smarty->assign('version',file_get_contents(LSHX_BLOG_ROOT.'home/version.txt'));
...
elseif ($action=='get_version')
{
	...
	$json   = new JSON;
	$notice=file_get_contents('https://blueblog.lansehangxian.com/api/getnew.php?act=version&u='.str_replace(LSHX_BLOG_WS_ADMIN, '', dirname($url)));
