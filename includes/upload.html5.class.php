...
class cls_upload {
	...
	function upload($upload,$type='img',$upload_dir='home/upload',$mkdir_type='month')
	{
		...
		$dir=LSHX_BLOG_ROOT.$upload_dir.'/'.$mkdir.'/';
		$url_dir=$upload_dir.'/'.$mkdir.'/';
		...
		//对html5文件上传方式进行处理
		if(isset($_SERVER['HTTP_CONTENT_DISPOSITION'])&&!empty(ini_get("upload_tmp_dir")))
		{
		...
		else
		{
			...
			{
				$this->error='此文件类别不允许上传222'.$upload[file_name];
				return false;
			}
	...
	{
		$allow_exp='txt,rar,zip,jpg,jpeg,gif,png,bmp,swf,flv,wmv,avi,wma,mp3,mid,sql';
	...
	function check_type($file_name,$type='img')
	{
		...
		$file_types['file'] = 'zip|doc|rar|pdf|txt|mp3|wav|mpeg|sql|tar.gz|.7z';
