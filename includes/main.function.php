...
//输出消息格式
function show_message($msg,$link_url,$noticetype=1)
{
...
function insert_member_info()
{
	...
	if ($_SESSION['user_id'] > 0)
	{
		...
		if ($user['group_id']==1)
		{
			$admin_url='<li><a href="'.$domain.LSHX_BLOG_WS_ADMIN.'"/>后台管理</a></li>';
		}
...
function insert_comments_name()
{
	...
	{
		$user=user_info();
		$content='<div class="commentFormRow">名字：'.$user['user_name'].'</div><input type="hidden" name="name" id="name" value='.$user['user_name'].' /></p>'.
		'<input type="hidden" name="url" id="url" value="'.$user['home'].'" />'.
		'<input type="hidden" name="email" id="email" value="'.$user['email'].'" />';
	}
	else
	{
		$content=' <div class="commentFormRow">
                  <input name="name" id="name" type="text" class="inputText" placeholder="姓名" />
                </div>
                <div class="commentFormRow">
                  <input  name="url" id="url" type="text" class="inputText" placeholder="个人主页地址" />
                </div>
                <div class="commentFormRow">
                  <input name="email" id="email" type="text" class="inputText" placeholder="E-mail" />
                </div>';
	}
	return $content;
}
...
function hook($id,$arr=array())
{
	...
	foreach ($plugins_list as $val)
	{
		...
		if (file_exists(LSHX_BLOG_ROOT. '/home/plugins/' . $val['plugin_id'].'/do.php'))
		{
			include_once(LSHX_BLOG_ROOT. '/home/plugins/' . $val['plugin_id'].'/do.php');
			$plugin_do($arr);
		}
	}
}
...
function sl_config()
{
	if (SLSYS=='NORMAL')
	{
		if (file_exists(LSHX_BLOG_ROOT.'/home/cache/sl_config.php'))
		{
			$sc_tmp=file_get_contents(LSHX_BLOG_ROOT.'/home/cache/sl_config.php');
			$config=unserialize($sc_tmp);
		}
		else
		{
			$config=get_sl_config();
			$fp=@fopen(LSHX_BLOG_ROOT.'home/cache/sl_config.php',"w");
			flock($fp,LOCK_EX);
